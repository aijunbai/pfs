...
Gaussian::Gaussian():
    mLoaded(false)
{
  if (Params::ins().gaussian_approximate) {
    ifstream fin("./src/gaussian.tbl");
    ...
    if (fin.good()) {
      ...
      if (i != 5000) {
        PRINT_ERROR("gaussian.tbl file error!");
      }
      else {
        mLoaded = true;
      }
...
Gaussian &Gaussian::ins()
{
  static Gaussian gaussian;
  return gaussian;
}

double Gaussian::cdf(const double mu, const double segma, double x)
{
  if (!mLoaded) {
    boost::math::normal_distribution<> dist(mu, segma);
  ...
  if (x >= 0.0) {
    ...
    int idx = floor(tmp);
    double prob = mCDF[idx];

    if (idx < 4999) {
      prob += (mCDF[idx + 1] - mCDF[idx]) * (tmp - idx);
    }

    return prob;
  }
  else {
    return 1.0 - cdf(0.0, 1.0, -x);
  }
}

double Gaussian::pdf(const double mu, const double segma, double x)
{
  if (!mLoaded) {
    boost::math::normal_distribution<> dist(mu, segma);
  ...
  double p1 = cdf(mu, segma, x + h);
  double p2 = cdf(mu, segma, x - h);
  double p = (p1 - p2) / (2.0 * h);
...
void SimpleRNG::RandomSeeding(int seed)
{
  ...
  for(int i = 0; i < HashSize; ++i) {
    mStatus_rand[i] = seed + i;
    srand48_r(seed + i, &mStatus_drand48[i]);
  }
}

int SimpleRNG::GetRand()
{
  if (Params::ins().threads) {
    int id = hash(pthread_self()) % HashSize;
    return rand_r(&mStatus_rand[id]);
  }
...
double SimpleRNG::GetUniform(double low, double high)
{
  if (Params::ins().threads) {
    double ret;
    int id = hash(pthread_self()) % HashSize;
    drand48_r(&mStatus_drand48[id], &ret);
